...
const SemiCircle3dots = ({ variant, text, angle }: SemiCircleProps) => {
    return (
        <div className={`flex justify-center items-center h-36 w-60 bg-indigo-500 rounded-tl-full rounded-tr-full transform rotate-${angle}`}>
            {variant === "dots" && (
                <div>
            ...
            {variant === "text" && text !== undefined && (
                <div className="text-white font-bold transform rotate-180 text-3xl uppercase" style={{ transform: `rotate(${angle}deg)` }}>
                    {text}
                </div>
            )}
        </div>
