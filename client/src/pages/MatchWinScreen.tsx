...
const MatchWinScreen = ({playerScore = 3, opponentScore = 2, playerName = "SPONGEBOB", opponentName = "PATRICK", spectatorCount = 32}: MatchWinScreenProps) => {
    return (
        <div>
            ...
            <div className='text-white font-bold text-6xl fixed top-[51%] mx-auto max-w-max inset-x-0'>
                <h1><span className='text-[#65DB71]'>{playerScore}</span> - {opponentScore}</h1>
            </div>
            <div className='text-white font-bold text-2xl md:text-3xl fixed top-[66%] mx-auto max-w-max inset-x-0'>
                <p><span className='text-[#FFC700]'>{opponentName}</span> AND THEIR FOLLOWERS WILL NOW FOLLOW YOU!</p>
            </div>
